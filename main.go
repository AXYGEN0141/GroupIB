...
// message if port not specified
var (
	port = flag.String("port", "", "Choose the port to run service on")
)
...
func main() {
	flag.Parse()
	// if port is empty show how to use cli
	if *port == "" {
		log.Printf("missing port argument")
		usage()
	}
